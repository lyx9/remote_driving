    ...
    <header class="replay-header">
      ...
      <div class="header-center">
        ...
        <div class="bag-info" v-if="currentBag">
          ...
          <span class="bag-meta">
            {{ (currentBag.size / 1024 / 1024 / 1024).toFixed(2) }} GB
            <span v-if="currentBag.format"> | {{ currentBag.format }}</span>
          </span>
        </div>
      </div>
          ...
          <div v-else class="bag-list">
            ...
            >
              <div class="bag-item-header">
                ...
                <span class="bag-item-size">{{ (bag.size / 1024 / 1024 / 1024).toFixed(2) }} GB</span>
              </div>
              <div class="bag-item-meta">
                <span v-if="bag.message_count">{{ bag.message_count.toLocaleString() }} messages</span>
                <span v-if="bag.db3_files">{{ bag.db3_files }} db3 files</span>
                <span>{{ bag.format || 'ROS2' }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="topic-list">
          ...
          >
            ...
            <div class="topic-info">
              ...
              <div class="topic-count">{{ topic.message_count || topic.messageCount || 0 }} msgs</div>
            </div>
          </div>
        </div>
      </aside>
      ...
      <aside class="control-panel">
        <div class="panel-section">
          ...
          <div class="playback-info">
            <div class="info-row">
              <span>Messages:</span>
              <span class="info-value">{{ (messageCount || 0).toLocaleString() }}</span>
            </div>
            <div class="info-row">
              <span>Status:</span>
...
async function refreshBagList() {
  try {
    console.log('[RosBagReplayPro] Refreshing bag list...')
    await streamService.listBags()
    console.log('[RosBagReplayPro] Bag list refreshed, count:', availableBags.value.length)
  } catch (error) {
    console.error('[RosBagReplayPro] Failed to refresh bag list:', error)
  }
}
