...
继续优化代码以及补充未完成的功能，把ai建议的看板进行美化，并接入豆包之类的LLM模型，给出配置的a[i接口，其次进行场景和驾驶建议的分析；此外，地图侧接入高德地图的api和界面，用于显示车辆位置信息。此外，我需要建立一个自动驾驶的远程平台的演示的视频，从1.平台登陆 2.显示工作界面 3.接入ad的自动驾驶的本地视频，4.mock被控的100车辆车队进入自动驾驶，展示根据优先级排序 5.task 1 mock车队数据，车队提示风险异常，前端界面可选择可选择是否要进入接管，以及显示需要接管的原因，进行调度算法排序后，在主界面可选择然后选择接管后，mock ai给出的车辆事故和原因分析，比如，前方障碍物需要绕行，建议接管 6.task 2 ，在车辆行使过程中，新增remote assistant功能，实时给出主页面选中的车辆的自动驾驶的操作建议 7.远程接入，进行避险 8.显示存储到平台2数据库平台的过程